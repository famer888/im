...
export const getChannelUsers = (data) => {
  return requestAxios(`/channel/channelMember/pageChannelNormalMember`, data , {
        headers: {
            ...getSignHeader()
        }
    });
...
export const getGameGlobalConfig = (data) =>
    requestAxios(`/channel/getChannelById`,{
...
function requestAxios(url, params, opts) {
    ...
    console.log("requestAxios--", reqBody, url)
...
const aesDecrypted = (decodedBody) => {
     // Decrypt response
    const cipher = crypto.createDecipheriv('aes-128-ecb', Buffer.from(bodyAesKey), null);
...
function aesDecode(encryptedData) {
    ...
    // 创建解密器
    const decipher = crypto.createDecipheriv('aes-128-ecb', Buffer.from(bodyAesKey), null);
