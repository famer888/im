    ...
    <div>
      ...
      <div class="member-list">
        <div v-if="hostInfo" class="member-item cursor">
          <ComImage
            :src="hostInfo.icon || hostInfo.user?.icon"
            type="friend"
            class="member-avatar"
          />
...
<script>
// api
import { groupRemoveAdmin } from "@/api/imGroup";
...
export default {
  ...
  methods: {
    ...
    handelHostInfoSet() {
      ...
      // 登录信息是否为群主
      this.loginIsHost = longToNum(this.hostInfo.id) === loginId;
    },
    removeManage(item) {
      const groupId = this.groupId;
      const adminUid = item.id;

      if (!groupId || !adminUid) {
        return;
      }

      groupRemoveAdmin({ groupId, adminUid }).then((res) => {
    ...
    getItemName(item) {
      return (
        item.name ||
        item.nickName ||
        item.user?.friendRelation?.remarkName ||
        item.groupNickName ||
        item.user.nickName
      );
    },
