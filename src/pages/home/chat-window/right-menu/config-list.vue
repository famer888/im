<template>
  <ul class="comConfigList" v-if="showChannelConfig">
    ...
    <template v-else>
      <li class="disturb" v-if="chatContent.id !== 9701">
        <span> {{ $t("消息免打扰") }}</span>
    ...
    <template v-if="!isChannel && !isGroup && chatContent.memberType !== 2 && ![9700, 9701].includes(Number(chatContent.id))">
      <li>
        <span> {{ $t("阅后即焚") }}</span>
        <ComSwitch
          :value="bfReadCancel"
    ...
    <template v-if="!isChannel && !isGroup && ![9700, 9701].includes(Number(chatContent.id))">
      <li>
        <span>加入黑名单</span>
    ...
    <template v-else-if="![9700, 9701].includes(Number(chatContent.id))">
