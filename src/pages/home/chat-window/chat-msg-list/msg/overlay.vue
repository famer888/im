<template>
  <div class="overlay" v-if="showOverlay">
    ...
    <!-- Loading 状态：居中按钮区域 -->
    <div class="center-control" v-if="showCenterControl">
      <!-- 进度环或旋转环 -->
      <div class="progress-ring" :class="{ spinning: loading && !hasPercent }">
    ...
    <div class="status-overlay" v-if="showStatus">
      <div class="status-content">
        <!-- 过期 icon -->
        <svg v-if="isExpired" class="status-icon" width="28" height="28" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- 无法加载 icon -->
        <svg v-else class="status-icon status-icon-error" width="34" height="25" viewBox="0 0 28 21" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  name: 'Overlay',
  props: ['loading', 'percent', 'status', 'duration', 'extension', 'isVideo'],
  computed: {
    showOverlay() {
      return this.loading || this.showStatus || this.isVideo;
    },
    showCenterControl() {
      // 有错误状态时不显示中央控制区域
      // 视频：始终显示中央控制区域（圆环和播放按钮）
      // 非视频：仅在 loading 时显示
...
/* 透明遮罩 27% 透明度 (白底上呈现 #BABABA / 186,186,186) */
.mask {
  position: absolute;
  top: 0;
  left: 0;
...
.progress-ring {
  ...
  height: 48px;
  // 背景只覆盖内圈，不要超过最外层圆环
  // 按钮叠加 mask(0.27) + 白色背景，目标 #828282 / 130,130,130
  &::before {
    content: '';
    position: absolute;
    inset: 4px;
...
/* Status 错误/过期状态 5% 透明度 (白底上呈现 #F2F2F2) */
.status-overlay {
  position: absolute;
  top: 0;
  left: 0;
